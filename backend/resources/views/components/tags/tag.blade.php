<div {{ $attributes->merge(['class' => "flex items-center justify-between"]) }}>
    ...
    <div class="flex space-x-1 items-center">
        <button @click="showEditTag(tag)" type="button"
        ...
        <button @click="deleteTag(tag)" type="button" :disabled="tagRequestInProgress[tag.id]"
                class="relative flex items-center justify-center whitespace-nowrap rounded-full p-1.5 text-red-500 font-semibold outline-0 outline-transparent hover:text-white hover:bg-red-500 focus:outline-2 focus:outline-offset-2 focus:outline-red-600 transition"
                :class="{
        ...
        >
            ...
            <span class="sr-only" x-show="!tagRequestInProgress[tag.id]"
                  x-text="tagPreventDelete[tag.id] ? 'Supprimer' : 'Vraiment ?'"
            ></span>
        </button>
    </div>
</div>
